  ...
  function chartInner(selection){
    ...
    chartInner.update = function(source) {
          ...
          if (radarData[0]){
            ...
            if(radarData[0].selfEdge){
              d3.select(this).append('svg:image')
                .attr({
                  'xlink:href': '../static/img/Clockwise_arrow.svg',  // can also add svg file here
                  x: 80,
                  y: 80,
                  width: 28,
                  height: 28
      ...
      nodeEnter.append("text")
        ...
        .text(function (d) {
            var info = d.data.nodeInfo
            if(info) {
                return info['metadata.general.name'];
            }
            return 'workload';
        });
    ...
    function click(d) {
      ...
      chartInner.update(d);
      var name = d.data.nodeInfo['metadata.general.name'];
      if(name === 'workload') {
          showWorkLoadProperties('workload');
      }
      for (var i in elementInformation) {
          if(elementInformation[i].name === name) {
              sideNavShowElementProperty(i);
          }
      }
